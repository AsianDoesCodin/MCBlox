...
pub async fn install_forge(
    ...
    // Download Forge libraries. Modern Forge splits some runtime libraries between
    // version.json and install_profile.json; the launcher needs both or Forge can
    // crash before mod discovery with missing fmlcore/javafmllanguage jars.
    let mut forge_libs = Vec::new();
    let mut seen_forge_libs = HashSet::new();
        ...
        for lib in libs {
            ...
            let url = if let Some(downloads) = lib["downloads"]["artifact"]["url"].as_str() {
                if downloads.is_empty() {
                    ...
                    if !jar_path.exists() {
                        ...
                        let extracted = extract_from_zip(&installer_path, &inner_path, &jar_path)?;
                        if !extracted {
                            return Err(format!("Required Forge library {} was not found in installer at {}", name, inner_path));
                        }
                    }
                    if seen_forge_libs.insert(jar_path.clone()) {
                        forge_libs.push(jar_path);
                    }
                    continue;
                }
                downloads.to_string()
            ...
            if !lib_path.exists() {
                if let Err(e) = download_file(client, &url, &lib_path).await {
                    return Err(format!("Required Forge library {} failed to download: {}", name, e));
                }
            }
            if seen_forge_libs.insert(lib_path.clone()) {
                forge_libs.push(lib_path);
            }
        }
    }

    if let Some(libs) = install_profile["libraries"].as_array() {
        for lib in libs {
            let name = lib["name"].as_str().unwrap_or("");
            if name.is_empty() { continue; }
            ...
            let url = if let Some(downloads) = lib["downloads"]["artifact"]["url"].as_str() {
                if downloads.is_empty() {
                    if !lib_path.exists() {
                        let inner_path = format!("maven/{}", artifact_path);
                        let extracted = extract_from_zip(&installer_path, &inner_path, &lib_path)?;
                        if !extracted {
                            return Err(format!("Required Forge install-profile library {} was not found in installer at {}", name, inner_path));
                        }
                    }
                    String::new()
                } else {
                ...
                format!("https://maven.minecraftforge.net/{}", artifact_path)
            };

            if !url.is_empty() && !lib_path.exists() {
                download_file(client, &url, &lib_path).await
...
fn maven_to_path(name: &str) -> String {
    // Convert Maven coordinates to a repository path.
    // Supports optional classifier: "group:artifact:version:classifier".
